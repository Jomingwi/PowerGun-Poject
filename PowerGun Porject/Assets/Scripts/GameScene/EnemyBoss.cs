...
public class EnemyBoss : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isPattern == false)
        {
            ...
            if (patternTimer >= 3) //패턴간격
            {
                nextPattern(gameManager.curDifficulty);
                patternTimer = 0f;
            }
    ...
    private void executePattern()
    {
        ...
        timer = 0;
        patternActive = false;
        isPattern = false; // 패턴 완료후 움직임
        isbossMoving = true; // 패턴실행후 이동
        isbossDash = false;
        isJump = false;
